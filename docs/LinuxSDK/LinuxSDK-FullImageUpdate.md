...
- [Introduction](https://developer.afero.io/LinuxSDK-FullImageUpdate#intro)

- - [OTA Image Concepts](https://developer.afero.io/LinuxSDK-FullImageUpdate#otaconcepts)
  - [The Journey of an OTA Image](https://developer.afero.io/LinuxSDK-FullImageUpdate#otajourney)
  - [Use Case](https://developer.afero.io/LinuxSDK-FullImageUpdate#otausecase)
  - [OTA Design and Best Practices](https://developer.afero.io/LinuxSDK-FullImageUpdate#otadesign)

- [The OTA Image Update Steps](https://developer.afero.io/LinuxSDK-FullImageUpdate#stepoverview)

- - [1. Create the OTA Image](https://developer.afero.io/LinuxSDK-FullImageUpdate#createimage)
  - [2. Deploy the OTA Image to a Device](https://developer.afero.io/LinuxSDK-FullImageUpdate#deployimage)
  - [3. Install Image on Device Using otamgr Daemon](https://developer.afero.io/LinuxSDK-FullImageUpdate#otamanager)

- [OTA Python Tools Reference](https://developer.afero.io/LinuxSDK-FullImageUpdate#otatoolref)
...
![OTA Journey](https://developer.afero.io/static/custom/images/OTA-Journey.png)
...
![OTA Image Partitions](https://developer.afero.io/static/custom/images/OTAImagePartitions.png)
...
Follow the instructions outlined below to perform a full OTA image update. (The steps include the use of the Python script tools Afero provides. These scripts are described in detail in the section, [OTA Python Tools Reference](https://developer.afero.io/LinuxSDK-FullImageUpdate#otatoolref)).
...
2. [Deploy the OTA image](https://developer.afero.io/LinuxSDK-FullImageUpdate#deployimage) by pushing the OTA image file to the Linux device.

3. [Install the OTA image](https://developer.afero.io/LinuxSDK-FullImageUpdate#otamanager) on the device using the otamgr daemon.
...
- For the Atmel SAMA5D2, follow the instructions in [Build the SAMA5D2 Linux OS Image](https://developer.afero.io/LinuxSDK-PotencoSAMA5D2#bldpotosimage) to set up the SAMA5D2 Yocto build.
- For the BeagleBone Green Wireless, follow the instructions in [Build the BBGW Linux OS Image](https://developer.afero.io/LinuxSDK-PotencoBBGW#bldpotosimage) to set up the BBGW Yocto build.
...
Read details in [About the Configuration File](https://developer.afero.io/LinuxSDK-FullImageUpdate#configfile). Use your favorite editor to edit the configuration file `partner-ota-conf.json`. Fill in the following fields and save it:
...
Read details in the reference sections below, [About the OTA Record](https://developer.afero.io/LinuxSDK-FullImageUpdate#otarecord) and [Using the partner-ota-hub-uploader.py Script](https://developer.afero.io/LinuxSDK-FullImageUpdate#stage1createrecord).
...
1. Make sure you have already cloned the Afero partner tools and edited the configuration file, as described above in [Set Up the Environment](https://developer.afero.io/LinuxSDK-FullImageUpdate#setupenv) and [Customize the Configuration File](https://developer.afero.io/LinuxSDK-FullImageUpdate#editconfigfile).
...
Once the OTA image has been deployed to the device and hubby has informed the otamgr daemon (via an Afero attribute) the location of the validated firmware in the filesystem, the otamgr daemon can install the image on the device. Please refer to [OTA Manager (otamgr)](https://developer.afero.io/LinuxSDK-Overview#otamgrSummary) and [OTA Manager Daemon](https://developer.afero.io/LinuxSDK-OTAManager) for details.
...
The following fields are security credentials that are used to access the user’s account. The `username`, `auth-string`, and `partnerId` can be obtained from the Afero Profile Editor. Please refer to [Cloud APIs](https://developer.afero.io/CloudAPIs) for more details.
...
```
 ...
 Next: [Afero Potenco Setup Guides](https://developer.afero.io/LinuxSDK-PotencoSetupGds)
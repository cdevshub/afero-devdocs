...
2. In the far-right pane, click + ATTRIBUTE OPTION. The Attribute Option Details dialog box opens:![Attribute Option Details Dialog Box](img/APE-AttrOptDetails.png)
...
3. Complete each field, as relevant to the targeted UI control(s), using the table below as reference:
   ...
   | Primary Operation                                            | On/Off checkbox, **only** available for write attributes that toggle between two values, and for controls that support writing to an attribute. Only one of your UI controls can be set as Primary Operation within a given device Profile.With this option **selected**, you’ll be able to control the attribute from the mobile app Home screen by double-tapping the device icon. Double-tapping will toggle between the attribute values. | MenuSliderTemperatureBattery LevelSwitch   |
...
4. When you have finished defining the details for a given attribute, click OK to save and dismiss the dialog box. Your saved attribute options will all be listed in the far-right column labeled ATTRIBUTE OPTIONS. Each attribute option is identified with its *Default Label:Attribute* in the upper-left of each pane:![Attribute Options Pane](img/APE-AttrOptionsPane.png)

5. You can take the following actions on any of your saved attribute options:

6. - View a summary of your attribution option details by hovering over the info icon![Info Icon](img/APE-InfoIcon.png).
   - Edit saved attribute option details by selecting the pencil icon![Pencil Icon](img/PencilIconWhite.png)in the relevant Attribute Option pane: The Attribute Option Details dialog box will open for edit.
   - You can delete an attribute option by selecting the trash icon![Trash Icon](img/TrashIconWhite.png).
...
2. You’ll define your groups in the groups ribbon:
   ...
   3. To **reposition** a group in the ribbon, click the group to make it active, then hover your cursor to the left of the group name (outlined in orange) to reveal the textured move handle![img](img/APE-MoveGroup.png)then drag & drop the group – left or right – to the new position.
   4. To **delete** a group, click the group to make it active, then hover your cursor to the right of the group name to reveal the trash icon![img](img/APE-GroupWithTrashCan.png)then click the trash.
...
4. To quickly switch a control’s View Style or remove it from the group, hover your cursor to the right of a control that’s been added to a group, then click the pencil icon that appears. Editing tools open:![img](img/APE-EditControl.png)

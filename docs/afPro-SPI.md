...
This sequence forces ASR to start from a known (initial) state. ASR will initiate the zero sync right after it reboots, as described below in [Example 1](https://afero-devdocs.readthedocs.io/en/latest/afPro-SPI/#example-1-zero-sync).
...
- [Example 1: Zero Sync](https://afero-devdocs.readthedocs.io/en/latest/afPro-SPI/#example-1-zero-sync) - Occurs when the master sends a Sync Request to the slave, but the data payload is empty. For example, when the master boots, it sends a zero Sync Request to the slave.
- [Example 2: Set Attribute](https://afero-devdocs.readthedocs.io/en/latest/afPro-SPI/#example-2-set-attribute) - Occurs when the master sets the value of an attribute.
- [Example 3: Update Attribute](https://afero-devdocs.readthedocs.io/en/latest/afPro-SPI/#example-3-update-attribute) - Occurs when the MCU changes the value of an attribute and sends the update back to ASR.
- [Example 4: Set Attribute with Collision](https://afero-devdocs.readthedocs.io/en/latest/afPro-SPI/#example-4-set-attribute-with-collision) - Occurs when the master and slave both want to send a request.
